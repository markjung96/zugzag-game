...
_Started 2026-05-13 09:09 UTC_
...
Project title is now "zugzag-game"
...
<attached_files>
- uploads/DESIGN_PROMPT.md
- uploads/DESIGN.md
- uploads/FEATURE_SPEC.md
- uploads/PRINCIPLES.md
</attached_files>

<pasted_text name="Pasted text (803 lines)">
...
Built zugzag-game Phase 1 — **12 screens + 9-state TV matrix + primitive library + interactive state-machine prototype**, all on a single pan/zoom design canvas.

**Aesthetic system (declared up front, applied throughout):**
- Dark mode only — `#0A0A0B` canvas, surface tiers via tone (no decorative shadows)
- Pretendard (한글) + Inter (numerics) — italic banned, weight + size for emphasis
- `#FF3B5C` ranked / `#FFD24A` champion-glow / true climbing-hold hexes (purple confined to holds)
- 4px spacing grid, asymmetric radius scale (4 / 8 / 16 / 24 / pill)

**Sections on canvas:**
1. **Primitives** — Session Kind Badge (RANKED solid / CASUAL outline), FAB, Hold Color Chip (60×60 min, white/black get outlines), Toasts, Avatars
2. **P1 + P10 TV (1920×1080)** — live / loading / empty / reconnecting / pre-session / ceremony / no-data / expired / revoked
3. **Interactive P5** — state picker + live phone with working modal launches
4. **P5 state machine** — 7 hero variants in dedicated phones (no card dashboard)
5. **P2 ProblemBoard** — live / loading / empty
6. **P3 Send Modal** — all 3 steps × ranked vs casual × stale/offline score-preview states
7. **P4 SeasonDetail** — ranking-first, my-strip thin, medal rings on 1-3
8. **P6/P7/P8 Admin** — linear season form with sticky summary, quick-ranked bottom sheet w/ QR success, TV token desktop
9. **P9 AdminDashboard** — KPI strip → sparkline → CSV → table
10. **P11 OG cards** — Champion (1200×630, win-glow) and Personal (3-stat grid + sparkline) — pure data posters, no photo backgrounds
11. **P12 SharedSeasonLanding** — mobile, Top 3, sticky "크루 만들기" CTA

**Blacklist sweep:** no cream/serif/italic/terracotta, no violet gradients, no 3-icon-circle grids, no centered-everything, no uniform bubbly radii, no decorative blobs, no emoji glyphs, no "Welcome to..." copy. P5 is a state machine (not a card dashboard). Session Kind Badge appears in every spec-required location. P3 CASUAL mode never shows a score preview.

Drag artboards to reorder, click any to enter focus mode, use the state buttons in section 3 to drive the interactive prototype.
